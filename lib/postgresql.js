...
function PostgreSQL(postgresql, settings) {
  ...
  for (var key in settings) {
    if (settings.hasOwnProperty(key)) clientConfig[key] = settings[key];
  }
  if (settings.url) clientConfig.connectionString = settings.url;
  if (!clientConfig.max) clientConfig.max = 10;
  this.pg = new postgresql.Pool(clientConfig);
  this.settings = settings;
  if (settings.debug) {
...
PostgreSQL.prototype.executeSQL = function(sql, params, options, callback) {
  ...
  if (transaction && transaction.connection &&
    ...
    debug('Execute SQL within a transaction %d', transaction.connection.processID);
    executeWithConnection(transaction.connection);
  } else {
    self.pg.connect(function(err, connection, done) {
      if (err) return callback(err);
      debug('Execute SQL without a transaction %d', connection.processID);
...
PostgreSQL.prototype.buildExpression = function(columnName, operator,
  ...
  switch(operator) {
    ...
    case 'contains':
      if (propertyDefinition.type.name === 'Array') {
        return new ParameterizedSQL(columnName + ' @> ?',
          [[operatorValue]]);
      }
    default:
...
PostgreSQL.prototype.toColumnValue = function(prop, val) {
  ...
  if (prop.type.name && prop.type.name.toLowerCase() === 'ObjectID'.toLowerCase()) {
    return String(val);
  }
